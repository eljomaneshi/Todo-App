package com.eljo.rest.webservices.restfulwebservices.jwt;
...
import com.eljo.rest.webservices.restfulwebservices.usermanagement.User;
import com.eljo.rest.webservices.restfulwebservices.usermanagement.UserRepository;
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity, HandlerMappingIntrospector introspector)
		...
		return httpSecurity
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers("/authenticate").permitAll()
						.requestMatchers("/register").permitAll().requestMatchers("/h2-console/**").permitAll()
	...
	public UserDetailsService userDetailsService() {
		return username -> {
			...
			return org.springframework.security.core.userdetails.User.withUsername(user.get().getUsername())
					.password("{noop}" + user.get().getPassword()).roles(user.get().getRole()).authorities("ROLE_ADMIN")
					.build();
		};
	}
